    ...
    if (continueButton) {
        continueButton.addEventListener('click', function (event) {
            ...
            if (navn && efternavn && adresse && by && postnummer && land && telefon && email) {
                ...
                console.log('Ikke alle felter er udfyldt. Viser error toast-notifikation.');

                const errorToast = document.getElementById('toast-notification-error');
                if (errorToast) {
                    errorToast.textContent = "Udfyld venligst alle felter";
                    errorToast.classList.add('toast-visible', 'toast-error');
                    console.log('Error toast-notifikation blev vist.');

                    setTimeout(function () {
                        errorToast.classList.remove('toast-visible', 'toast-error');
                        console.log('Error toast-notifikation blev fjernet.');
                    }, 4000);
                } else {
                    console.error('Error toast-notifikationen blev ikke fundet i DOM\'en.');
                }
            }
        });
    } else {
